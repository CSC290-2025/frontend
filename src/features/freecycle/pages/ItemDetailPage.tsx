...
export default function ItemDetailPage() {
  ...
  const currentUserId: any = currentUser?.id;
  ...
  const handleRequest = async () => {
    if (!item || !currentUserId) {
      if (!currentUserId) alert('Please log in to request an item.');
      return;
    }
    ...
    const payload = {
      postId: item.id,
      receiverId: currentUserId as number,
    };

    console.log('--- Frontend Log: Sending Request Payload ---');
    console.log('Post ID:', payload.postId);
    console.log('Receiver ID (currentUserId):', payload.receiverId);
    console.log('-------------------------------------------');
    // ------------------------------------------
